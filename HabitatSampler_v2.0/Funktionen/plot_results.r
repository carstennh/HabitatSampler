...
##3.a.1##
setwd(inPath)
files<-grep(list.files()[grep(list.files(),pattern=".tif$")],pattern="step",value=T)
...
par(oma=c(0,4,0,0))
if (length(color) == 0) {pie(percent, labels =percent, col=col(numberHabitats+1), main="",cex=1.5,lwd=2,border="white",init.angle=45)
legend(x=-1.8,y=1.5,legend=append(classNames,"other"),fill=col(numberHabitats+1),cex=1.55,xpd=NA,bty="n")}else{
pie(percent, labels =percent, col=color, main="",cex=1.5,lwd=2,border="white",init.angle=45)
legend(x=-1.8,y=1.5,legend=append(classNames,"other"),fill=color,cex=1.55,xpd=NA,bty="n")}
}
